...
import VendorsSearchForm from './VendorsSearchForm';
...
class VendorsTable extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.vendorData != undefined) {
      var tmp = [];
      tmp.push({ partyId: nextProps.vendorData.partyId });
      this.setState({ vendorData: tmp });
    }
  }
  componentDidMount() {
  ...
  tableColumns = [
    ...
    {
      title: `${localConsts.ManifestCompanyName}`,
      dataIndex: 'Invoice Type',
      sorter: (a, b) => a.shortName.length - b.shortName.length,
    },
    {
      title: `${localConsts.ManifestCompanyTitle}`,
      dataIndex: 'Invoice Date',
    },
    {
      title: `${localConsts.ManifestLogoUrl}`,
    },
    {
      title: `${localConsts.ManifestPolicies}`,
    },
  ];
...
export default connect(({ Vendors_AP }) => ({
  data: [],
  vendorData: Vendors_AP.reducerSave,
}))(VendorsTable);
